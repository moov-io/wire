...
func (fib *FIBeneficiary) Parse(record string) (int, error) {
	...
	if fib.tag, read, err = fib.parseTag(record); err != nil {
		return 0, fieldError("FIBeneficiary.Tag", err)
	}
	length += read

	if read, err = fib.FIToFI.Parse(record[length:]); err != nil {
		return 0, err
	}
	length += read
...
func (fib *FIBeneficiary) String(options ...bool) string {
	...
	buf.WriteString(fib.FIToFI.String(isCompressed))
